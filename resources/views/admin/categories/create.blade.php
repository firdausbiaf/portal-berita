...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-stone-900">Tambah Kategori Baru</h2>
            <p class="text-xs text-stone-500 mt-1">Buat topik atau rubrikasi baru untuk artikel berita</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <input
                    ...
                    name="slug"
                    value="{{ old('slug') }}"
                    placeholder="otomotif (opsional, otomatis dibuat dari nama jika dikosongkan)"
                ...
                />
                <p class="text-[11px] text-stone-400 mt-1">Gunakan huruf kecil, angka, dan tanda hubung (-). Jika dikosongkan akan digenerate otomatis.</p>
                @error('slug')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
